...
#define FT_ORDER 661 // l'ordre de filter (nombre de coeifiicients de flter )

int x[FT_ORDER]; //tableau pour sauvegarder les valeurs entrants de l'ADC
...
//un tableau pour enregistrer les valeurs des coeifficients de filter
...
int main(void)
{
  ...
  while (1)
  {
		...
		adc_value = HAL_ADC_GetValue(&hadc1);// recevoir les valeurs de ADC
		x[660] = adc_value;
		HAL_DAC_SetValue(&hdac, DAC_CHANNEL_1, DAC_ALIGN_12B_R, (int)y);
		//envoyer les resultats vers le DAC cad vers l'oscillo
		delay_us(250);//ona besoin d'un delai
  }
